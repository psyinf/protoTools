...
ProtocolPublisher::~ProtocolPublisher() = default;

ProtocolPublisher::ProtocolPublisher(std::shared_ptr<zmq::context_t> context_ptr)
{
    _context_ptr = context_ptr;
    _pub_socket = std::make_unique<zmq::socket_t>(*_context_ptr, zmq::socket_type::pub);
}

void ProtocolPublisher::bind(const ProtoPublisherConfig& config)
{
...
void ProtocolPublisher::send(const ProtoHeader& header, const ProtoData& package)
{
    // format header
